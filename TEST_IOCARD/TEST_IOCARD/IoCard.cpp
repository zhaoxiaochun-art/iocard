...
BOOL CioControl::ReadParameterFile_NewLX45(const char* strPath)
{
	...
	sprintf_s(str,"基础参数");
	//计数器延迟参数,计数值延迟输出时间
	mCntVOutDelay = GetPrivateProfileInt(str,"CntVOutDelay",20,strPath);
	((CPci6356_ioCtrl*)m_pData)->Pci6356_SetDelayCntParam(mCntVOutDelay);
	//输出扩展配置参数,受控操作时控制增加输出高电平时间,_t为时间计数,_h为行计数
	mKeepExBase_t = GetPrivateProfileInt(str,"KeepTmExUnit",5,strPath);
	mKeepExBase_h = GetPrivateProfileInt(str,"KeepHcExUnit",5,strPath);
	((CPci6356_ioCtrl*)m_pData)->Pci6356_SetKeepExUnit(mKeepExBase_t,mKeepExBase_h);
	//与S500不同，各队列独自选择定时器与编码器选择方式，

	//设置编码器参数
	mCodehNum[0] = GetPrivateProfileInt(str,"InCodeh0",0,strPath);
	mCodehNum[1] = GetPrivateProfileInt(str,"InCodeh1",0,strPath);
	mCodehNum[2] = GetPrivateProfileInt(str,"InCodeh2",0,strPath);
	mCodehNum[3] = GetPrivateProfileInt(str,"InCodeh3",0,strPath);
	SetCodeFramelg_LX45();
	//设置编码器滤波系数
	int CodeFilter = GetPrivateProfileInt(str,"CodeFilter",85,strPath);//'b01010101=0x55=85,隐含滤波
	IOWrite(0x68,(BYTE)CodeFilter);	//设置级联输出端口
	mInCoeOut = GetPrivateProfileInt(str,"InCoeOut",0,strPath);
	if(!m_IfPcie32)	IOWrite(0x10,BYTE(mInCoeOut&0x0ff));
	mEnInIntFlag = GetPrivateProfileInt(str,"InEnINT",0,strPath);
	mInIntDelayms = GetPrivateProfileInt(str,"InDelayINTms",10,strPath);
	return TRUE;	
}
...
BOOL CioControl::ReadParameterFile_OLDS500(const char* strPath)
{
	...
	//计数器延迟参数,计数值延迟输出时间
		...
		mRunType1 = GetPrivateProfileInt(str,"RunType1",0,strPath);
		mRunType2 = GetPrivateProfileInt(str,"RunType2",0,strPath);
		((CPci6356_ioCtrl*)m_pData)->Pci6356_SetBaseType(mRunType1,mRunType2);
		mCode1dsType = GetPrivateProfileInt(str,"Code1dsType",0,strPath);
		mCode2dsType = GetPrivateProfileInt(str,"Code2dsType",0,strPath);
		mCode1hMax = GetPrivateProfileInt(str,"Code1hMax",1000,strPath);
		mCode2hMax = GetPrivateProfileInt(str,"Code2hMax",1000,strPath);
...
BOOL CioControl::PCI_Init(const char* strPath)
{
	char str[16],str1[32],strbd[32];
	int i,j;
	CPci6356_ioCtrl *pCard = ((CPci6356_ioCtrl*)m_pData);
	StopWork(TRUE);
	sprintf_s(str,"工作模型");
	//GetPrivateProfileInt(str,"BoardType",0,strPath);
	mWork_Mode = GetPrivateProfileInt(str,"WorkMode",0,strPath);
	((CPci6356_ioCtrl*)m_pData)->WorkMode = mWork_Mode;

	memset(strbd,0,32);
	GetPrivateProfileString(str,"BoardType","RunType1",strbd,30,strPath);
	CString sss;
	sss = strbd;
	...
	if((mWork_Mode==0)||(mWork_Mode==101)) {//基于2us时间计数器和编码器计数
		for (i=0;i<4;i++)
		{
			...
			pCard->m_LineCtrlParam[i].m_Used = GetPrivateProfileInt(str,"nUsed",0,strPath);//启用开关
			pCard->m_LineCtrlParam[i].m_InChn = GetPrivateProfileInt(str,"nInPort",0,strPath);//对应输入端口
			pCard->m_LineCtrlParam[i].m_InFilterType = GetPrivateProfileInt(str,"nInFilter",0,strPath);//对应输入端口
			pCard->m_LineCtrlParam[i].m_InMaxRate = GetPrivateProfileInt(str,"nMaxRate",10,strPath);//对应输入端口
//			pCard->m_LineCtrlParam[i].m_OutCtrlType = GetPrivateProfileInt(str,"nOutCtrlType",0,strPath);//输出隐含控制输出方式：独立/统一，已全部独立
			pCard->m_LineCtrlParam[i].m_OutNum = GetPrivateProfileInt(str,"nOutPortNum",1,strPath);//输出控制端口个数
			pCard->m_LineCtrlParam[i].m_TmType = GetPrivateProfileInt(str,"nRunType",0,strPath);//队列推进方式，LX45-0x18执行，不影响S500
			pCard->m_LineCtrlParam[i].m_LastOutPort = GetPrivateProfileInt(str,"nLastOutPort",-1,strPath);//最后一个控制端口
			pCard->m_LineCtrlParam[i].m_LineCntDelay = GetPrivateProfileInt(str,"nLineCntDelay",20,strPath);//计数延迟
			int cchn;

			for (j=0;j<pCard->m_LineCtrlParam[i].m_OutNum;j++)
			{
				sprintf_s(str1,"nOutPort%d",j);
				pCard->m_LineCtrlParam[i].m_OutChn[j] = GetPrivateProfileInt(str,str1,0,strPath);//输出端口
				cchn = pCard->m_LineCtrlParam[i].m_OutChn[j];
				pCard->m_OutportParam[cchn].m_outChn = cchn;
				sprintf_s(str1,"nDelay%d",j);
				pCard->m_LineCtrlParam[i].m_OutChnDelay[j] = GetPrivateProfileInt(str,str1,0,strPath);//延迟
				sprintf_s(str1,"nDelayIn%d",j);
				pCard->m_LineCtrlParam[i].m_OutChnDelayIN[j] = GetPrivateProfileInt(str,str1,0,strPath);//延迟输入产品个数

				sprintf_s(str1,"nDelayTm%d",j);
				pCard->m_LineCtrlParam[i].m_OutChnDelayTM[j] = GetPrivateProfileInt(str,str1,0,strPath);//延迟长度（产品间位置0-1023）
				sprintf_s(str1,"nKeep%d",j);
				pCard->m_LineCtrlParam[i].m_OutChnKeep[j] = GetPrivateProfileInt(str,str1,0,strPath);//输出保持时间长度
				sprintf_s(str1,"nType%d",j);
				pCard->m_LineCtrlParam[i].m_OutChnType[j] = GetPrivateProfileInt(str,str1,0,strPath);//类型
				...
				pCard->m_LineCtrlParam[i].m_OutChnCntDelay[j] = GetPrivateProfileInt(str,str1,20,strPath);//计数延迟
				...
				if( (pCard->m_LineCtrlParam[i].m_OutChnType[j] & 0x80)==0x80) {//输出需要序列
					...
					pCard->m_OutportParam[cchn].m_HighTm = GetPrivateProfileInt(str,str1,0,strPath);
					pCard->m_LineCtrlParam[i].m_OutHighTm[j] = pCard->m_OutportParam[cchn].m_HighTm;
					sprintf_s(str1,"nOutLow%d",j);
					pCard->m_OutportParam[cchn].m_LowTm = GetPrivateProfileInt(str,str1,0,strPath);
					pCard->m_LineCtrlParam[i].m_OutLowTm[j] = pCard->m_OutportParam[cchn].m_LowTm;
					sprintf_s(str1,"nOutReCycle%d",j);
					pCard->m_OutportParam[cchn].m_ReCycleNum = GetPrivateProfileInt(str,str1,0,strPath);
...
//		pCard->m_LineCtrlParam[i].m_OutCtrlType = GPci6356_DL_etPrivateProfileInt(str,"nOutCtrlType",0,strPath);//输出隐含控制输出方式：独立/统一
		pCard->m_LineCtrlParam[i].m_OutNum = GetPrivateProfileInt(str,"nOutPortNum",1,strPath);//个输出控制端口
		pCard->m_LineCtrlParam[i].m_TmType = GetPrivateProfileInt(str,"nRunType",0,strPath);//队列推进方式
		pCard->m_LineCtrlParam[i].m_LastOutPort = GetPrivateProfileInt(str,"nLastOutPort",-1,strPath);//最后一个控制端口 
		pCard->m_LineCtrlParam[i].m_LineCntDelay = GetPrivateProfileInt(str,"nLineCntDelay",20,strPath);//计数延迟
		int cchn;
		...
		for (j=0;j<pCard->m_LineCtrlParam[i].m_OutNum;j++)
		{
			...
			pCard->m_LineCtrlParam[i].m_OutChnDelayTM[j] = GetPrivateProfileInt(str,str1,0,strPath);//延迟长度（产品间位置0-1023）
			sprintf_s(str1,"nKeep%d",j);
			pCard->m_LineCtrlParam[i].m_OutChnKeep[j] = GetPrivateProfileInt(str,str1,0,strPath);//持续
			sprintf_s(str1,"nType%d",j);
			pCard->m_LineCtrlParam[i].m_OutChnType[j] = GetPrivateProfileInt(str,str1,0,strPath);//类型
			pCard->m_LineCtrlParam[i].m_OutChnKeep[j]*=500;
			sprintf_s(str1,"nOutCntDelay%d",j);
